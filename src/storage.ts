...
import type { AnnualReport, AppSettings, ReportArchive, ReportLock } from './types'
...
type LegacyReport = Partial<AnnualReport> & {
  steps?: number
  codingHours?: number
  photos?: number
  musicHours?: number
...
function normalizeData(data: Partial<StoredData>, seed: AnnualReport): StoredData {
  ...
  const migrationPending = demoMigrationPending || data.version !== 3
...
function readBrowser(seed: AnnualReport): StoredData {
  try {
    ...
    const lock = JSON.parse(localStorage.getItem(lockKey) ?? 'null') as ReportLock | null
    ...
    return normalizeData({ archive, lock, ...(draft ? { draft } : {}), settings }, seed)
  } catch {
    return normalizeData({ archive: { [seed.year]: seed }, lock: null }, seed)
  }
}

function writeBrowser(data: StoredData) {
  localStorage.setItem(archiveKey, JSON.stringify(data.archive))
  data.lock ? localStorage.setItem(lockKey, JSON.stringify(data.lock)) : localStorage.removeItem(lockKey)
...
export async function loadStoredData(seed: AnnualReport): Promise<StoredData> {
  ...
  try {
    ...
    return normalizeData(JSON.parse(raw) as Partial<StoredData>, seed)
  } catch {
    const initial = normalizeData({ archive: { [seed.year]: seed }, lock: null }, seed)
